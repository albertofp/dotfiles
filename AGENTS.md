...
```
flake.nix                        # Entry point — NixOS + home-manager + agenix + zen-browser + hyprpaper + rust-overlay
nixos/
  ...
  home/
    packages.nix                 # home.packages (user packages; Rust via rust-overlay)
...
.config/                         # Configs symlinked into ~ via dotfiles.nix (live-editable)
  ...
  ghostty/                       # Terminal
mac/                             # macOS-only configs and tooling (NOT used on NixOS)
  .zshrc                         # Shell config — NixOS uses programs.zsh in zsh.nix
  .tmux.conf                     # Tmux config (TPM) — NixOS uses programs.tmux in tmux.nix
  ansible/                       # Homebrew + Ansible bootstrap playbook
  .config/
...
### SSH agent
`services.ssh-agent.enable = true` runs one persistent agent per user session (declared in
`desktop.nix`). `programs.ssh.addKeysToAgent = "yes"` causes the key to be added on first use.
Do not use `eval $(ssh-agent -s)` anywhere in shell init — it creates duplicate agents.
...
### macOS split
All macOS-specific files live under `mac/`. On NixOS, none of these are used or symlinked.

- `mac/.zshrc` and `mac/.tmux.conf` — NixOS generates these via `programs.zsh` and `programs.tmux`
...
nixfmt nixos/home.nix nixos/system.nix nixos/home/*.nix nixos/system/*.nix nixos/lib/*.nix flake.nix
...
- Changes to the macOS setup under `mac/`
